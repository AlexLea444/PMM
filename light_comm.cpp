...
/*
 * Purpose: Receive High-Low-High-Low signal as specified for Challenge 1
 */
...
/*
 * Purpose: Send High-Low-High-Low signal as specified by PMM and Team Red *          Red for Challenge 4
 */
void Light_Comm::chall4_send() {
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 753; j++) {
      // Generate a high-amplitude square wave with f = 3kHz
      analogWrite(Pins::commLED, HIGH); // Set the output pin HIGH to represent an AM signal with a higher peak
...
/*
 * Purpose: Receive High-Low-High-Low signal as specified by PMM and Team Red *          Red for Challenge 4
 */
void Light_Comm::chall4_receive() {
  while (analogRead(Pins::commIn) < threshold) {}
  delay(250);
